...
function ContactUsPage() {
  ...
  const handleSubmit = async () => {
    const errors = {
      name:
        formUserData.name.length < 2 ? "الرجاء إدخال اسم صحيح" : "",
      phone_number:
      ...
      msg_content: !formUserData.msg_content.trim() ? "الرجاء إدخال محتوى الرسالة" : "",
    };
              ...
              <div>
                <div className="relative p-2">
                  <input
                    ...
                    onChange={(e) => handleChange("msg_content", e.currentTarget.value)}
                  />
                  <div className="absolute -top-1 right-5 bg-white">
                    محتوى الرسالة
                  </div>
                  {formErrors.msg_content && (
                    <p className="text-red-500 mt-2">{formErrors.msg_content}</p>
                  )}
                </div>
              </div>
        ...
        {isSubmitted && (
          <div className="flex justify-center space-x-5 items-center w-[800px] h-[350px] border-2 m-20 p-10 border-blue-950 rounded-lg">
            <div className="image-container w-1/2 pl-4">
              <img src={contactImage} alt="Description of Image" className="opacity-60"/>
            </div>
