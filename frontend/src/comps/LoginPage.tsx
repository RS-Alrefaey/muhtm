...
function Login() {
  ...
  const handleChange = (field: string, value: string) => {
    if (field === "username") {
      ...
      if (!value) {
        setFormErrors(prev => ({ ...prev, username: "فضلاً ادخل اسم المستخدم" }));
      } else if (/[^a-zA-Z0-9]/.test(value)) {
        setFormErrors(prev => ({ ...prev, username: "اسم المستخد غير صحيح" }));
      } else {
      ...
      if (!value) {
        setFormErrors(prev => ({ ...prev, password: "فضلا ادخل كلمة المرور" }));
      } else if (value.length < 8 || !/[A-Z]/.test(value) || !/[0-9]/.test(value)) {
        setFormErrors(prev => ({ ...prev, password: "كلمة المرور غير صحيحة" }));
      } else {
